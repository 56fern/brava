...
function GroupedProfileForm({ groupId, onCancel, onSave }: { groupId: string; onCancel: () => void; onSave: (profile: Profile) => void }) {
  ...
  return <FormCard title="New profile" onSubmit={submit} onCancel={onCancel}>
    ...
    <Field label="Cardholder name"><input required value={form.payment!.cardholderName} onChange={(event) => setPayment("cardholderName", event.target.value)} /></Field><Field label="Card brand"><select value={form.payment!.brand} onChange={(event) => setPayment("brand", event.target.value)}><option>Visa</option><option>Mastercard</option><option>Amex</option><option>Discover</option><option>Other</option></select></Field><Field label="Card number"><input required inputMode="numeric" autoComplete="off" maxLength={19} value={form.payment!.number ?? ""} onChange={(event) => setPayment("number", event.target.value.replace(/\D/g, "").slice(0, 19))} placeholder="Card number (stored encrypted)" /></Field><Field label="Last 4 digits"><input required inputMode="numeric" pattern="[0-9]{4}" maxLength={4} value={form.payment!.last4} onChange={(event) => setPayment("last4", event.target.value.replace(/\D/g, "").slice(0, 4))} placeholder="1234" /></Field><Field label="Expiry month"><input required inputMode="numeric" pattern="0[1-9]|1[0-2]" maxLength={2} value={form.payment!.expiryMonth} onChange={(event) => setPayment("expiryMonth", event.target.value.replace(/\D/g, "").slice(0, 2))} placeholder="MM" /></Field><Field label="Security code (CVV)"><input required inputMode="numeric" autoComplete="off" type="password" pattern="[0-9]{3,4}" maxLength={4} value={form.payment!.cvv ?? ""} onChange={(event) => setPayment("cvv", event.target.value.replace(/\D/g, "").slice(0, 4))} placeholder="•••" /></Field><Field label="Expiry year"><input required inputMode="numeric" pattern="[0-9]{4}" maxLength={4} value={form.payment!.expiryYear} onChange={(event) => setPayment("expiryYear", event.target.value.replace(/\D/g, "").slice(0, 4))} placeholder="YYYY" /></Field>
